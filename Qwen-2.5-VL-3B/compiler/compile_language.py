...
PROCESSOR_MODEL = os.environ.get(
    "PROCESSOR_MODEL_PATH",
    str(PRODUCT_ROOT / "checkpoint" / "Qwen2.5-VL-3B-Instruct"),
)
CALIBRATION = os.environ.get(
    "CALIBRATION_PATH",
    LEAP_ROOT + "/apis/calibration/calibration_data/mmstar/conversation.json",
)
ROTATION = os.environ.get(
    "ROTATION_PATH",
    str(PRODUCT_ROOT / "workspace" / "rotation" / "official_hidden_rotation_exact.pt"),
)
OFFICIAL_EMBED = os.environ.get("REFERENCE_EMBED_PATH", "")
OUTPUT = Path(
    os.environ.get(
        "OUTPUT_DIR",
        str(PRODUCT_ROOT / "workspace" / "build" / "language"),
    )
)
...
sys.path.insert(
    0,
    os.environ.get("QWEN_HELPER_ROOT", str(Path(__file__).resolve().parent)),
)
...
def compare_embed(candidate_path):
    ...
    print(
        f"[embed] candidate vs reference cosine={cosine:.9f} "
...
print("[1] loading Language model", flush=True)
...
print(
    f"[hidden-domain] transformed Language and calibration Vision; "
