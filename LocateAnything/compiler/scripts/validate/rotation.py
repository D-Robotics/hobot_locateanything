...
from leap_llm.models.locateanything.hidden_rotation import (
    build_signed_hadamard_rotation,
...
def build_language(model_path: str, rotate: bool, device: str):
    ...
    rotation = build_signed_hadamard_rotation(config.hidden_size)
    if rotate:
...
def validate_vision(model_path: str, device: str, dtype: torch.dtype) -> None:
    torch.manual_seed(20260718)
    inputs = torch.randn(1, 2304, 588, device=device, dtype=dtype)
    ...
    expected = raw_output @ build_signed_hadamard_rotation()
