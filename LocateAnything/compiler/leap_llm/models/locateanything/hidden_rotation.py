"""Signed normalized Hadamard transforms for LocateAnything hidden states.

The 2048-dimensional rotation is folded into model weights offline; no
rotation operator is added to the runtime graph.
"""
...
def build_signed_hadamard_rotation(hidden_size: int = REFERENCE_HIDDEN_SIZE) -> torch.Tensor:
    if hidden_size != REFERENCE_HIDDEN_SIZE:
        raise ValueError(
            "The signed normalized Sylvester Hadamard rotation is defined for "
            "hidden_size=2048; "
...
# Compatibility for existing imports. New code should use the mathematical name.
build_reference_hidden_rotation = build_signed_hadamard_rotation
...
def load_hidden_rotation(
    ...
    if path:
        ...
        rotation = build_signed_hadamard_rotation(hidden_size)
        source = SIGNED_HADAMARD_ROTATION_NAME
