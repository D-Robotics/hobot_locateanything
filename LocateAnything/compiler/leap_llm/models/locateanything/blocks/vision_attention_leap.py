...
from ..utils.rope_2d import rotate_adjacent_pairs
...
def rotate_half_leap(x):
    ...
    if len(shape) == 3:
        ...
        paired = leap.reshape(x, [n_local_head, seq_len, head_dim // 2, 2])
        real = leap.slice(
            paired, [0, 0, 0, 0], [n_local_head, seq_len, head_dim // 2, 1], [1, 1, 1, 1]
        )
        imag = leap.slice(
            paired, [0, 0, 0, 1], [n_local_head, seq_len, head_dim // 2, 2], [1, 1, 1, 1]
        )
        rotated = leap.concat([leap.mul(-1, imag), real], 3)
        return leap.reshape(rotated, [n_local_head, seq_len, head_dim])

    bs, n_heads, seq_len, head_dim = shape
    paired = leap.reshape(x, [bs, n_heads, seq_len, head_dim // 2, 2])
    real = leap.slice(
        paired, [0, 0, 0, 0, 0], [bs, n_heads, seq_len, head_dim // 2, 1], [1, 1, 1, 1, 1]
    )
    imag = leap.slice(
        paired, [0, 0, 0, 0, 1], [bs, n_heads, seq_len, head_dim // 2, 2], [1, 1, 1, 1, 1]
    )
    rotated = leap.concat([leap.mul(-1, imag), real], 4)
    return leap.reshape(rotated, [bs, n_heads, seq_len, head_dim])


def rotate_half_torch(x):
    return rotate_adjacent_pairs(x)
