...
bool LoadPayload(const std::string& token_path, const std::string& visual_path,
  ...
  if (payload->prompt_ids.size() > 1024) return false;
...
bool BuildPrefillEmbeddings(const rt::Graph& graph, const rt::EmbedLookup& embed,
                            ...
                            int32_t* active_len) {
  const auto& shape = graph.GetInputShapes()[0];
  if (graph.GetInputDtypes()[0] != kF16 || shape != std::vector<int32_t>{1, 1024, kHidden}) {
    return false;
  }
  output->shape = shape;
  output->dtype = kF16;
  ...
  if (payload == nullptr) {
    std::vector<int32_t> token_ids(1024);
    for (int32_t index = 0; index < 1024; ++index) token_ids[index] = index % kVocab;
    embed.Gather(token_ids.data(), 1024, output->data.data());
    *active_len = 1024;
    return true;
  }
...
bool BuildInputs(const rt::Graph& graph, const rt::EmbedLookup& embed,
  ...
  if (!embedding_ok ||
      !BuildPositions(graph, past_len, pbd,
                      payload != nullptr && graph.GetInputShapes()[0][1] == 1024
                          ? *active_len
                          : -1,
                      pbd_prefix_len,
                      &inputs->positions) ||
...
bool RunGraph(rt::HbmSession* session, const std::string& name,
  ...
  int32_t local_active_len =
      graph->GetInputShapes()[0].size() > 1 && graph->GetInputShapes()[0][1] == 1024
          ? 1024
          : -1;
...
bool RunHybridGenerationLegacy(rt::HbmSession* session,
  ...
  while (static_cast<int32_t>(response->size()) < max_new_tokens) {
    ...
    if (use_pbd) {
      ...
      if (!RunGraph(session, "decode", embed, 0, *history_len, true, *cache,
                    &outputs, nullptr, nullptr, &draft, false)) {
        return false;
      }
      ...
      if (!RunGraph(session, "decode", embed, 0, *history_len, false,
          ...
          !AppendCacheUpdate(committed, *history_len, cache, accepted)) {
        return false;
      }
    ...
    if (!RunGraph(session, "decode_ar", embed, 0, *history_len, false,
                  *cache, &outputs, nullptr, nullptr, &one, false) ||
        !AppendCacheUpdate(outputs, *history_len, cache)) {
      return false;
    }
    ++*history_len;
...
bool RunHybridGenerationFused(rt::HbmSession* session,
  ...
  while (static_cast<int32_t>(response->size()) < max_new_tokens) {
    ...
    if (use_pbd) {
      ...
      if (prefix_len > 0) {
        if (!AppendCacheUpdate(outputs, *history_len, cache, prefix_len)) {
          return false;
        }
      ...
      if (decision.switch_to_ar) {
        ...
        if (!RunGraph(session, ArGraphName(accepted), embed, 0, *history_len,
                      ...
                      &decision.tokens, false) ||
            !AppendCacheUpdate(bridge, *history_len, cache, accepted)) {
          return false;
        }
        *history_len += accepted;
        rt::Tensor next_logits;
...
bool RunPayload(rt::HbmSession* session, rt::EmbedLookup* embed,
  ...
  if (!RunGraph(session, "prefill", *embed, 0, 0, false, prefill_cache,
                &prefill_outputs, &payload, &active_len, nullptr, false)) {
    return false;
  }
...
int main(int argc, char** argv) {
  ...
  std::printf("[verdict] language HBM smoke %s\n", ok ? "PASSED" : "FAILED");
  return ok ? 0 : 11;
}
