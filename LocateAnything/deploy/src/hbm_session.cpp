...
Result Graph::Execute(hbDNNHandle_t handle,
  ...
  // The LocateAnything S600 profile is compiled for four Nash-P cores. The
  // board-verified launch path uses the explicit core-0..3 mask together with
  // HB_DNN_USER_DEFINED_L2M_SIZES=6:6:6:6. CORE_ANY is valid for the generic
  // reference runtime but caused the resident LA server to fail on this
  // board/runtime combination.
