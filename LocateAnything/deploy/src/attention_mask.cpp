...
bool BuildAttentionMask(int32_t q_len,
  ...
  const int32_t history_start = current_start - past_len;

  // The fixed graph drops the first q_len cache rows before appending the
  ...
  // Within the right-aligned current query window:
  ...
  // q_len), cols [cache_len-block_size, cache_len).
  if (block_size > 0 && !causal_attn && q_len >= block_size) {
    ...
    // (2) mask the previous round's trailing token. For fused q>B graphs,
    //     this is the last causal prefix row inside the current query window,
    //     so the upstream [-block_size-1] index must remain relative to the
    //     full KV width rather than to current_start.
