...
> **Reading rule.** Entries are chronological technical evidence, not a list of
> active defaults. References to 448, W4, cache 2048, 820 samples, removed
...
**Fix**: Added a downscale step in `examples/demo_min.py` that resizes any input whose long side exceeds 1024 px (`MAX_LONG_SIDE = 1024`), reducing token count to ~4000 and memory to a few hundred MB.
...
2. `grep "^class LayerNorm" compiler/leap_llm/nn/modules/layer_norm.py` 找到 leap-trace-aware 版存在。
...
**Evidence**:
1. `compiler/leap_llm/nn/modules/matmul.py::DynamicQuantMatmul.build`: `leap.block_quantized_matmul(x_q, y_q, x_s, y_s, mmaAlpha=1024.0)` — 是 "TransRhs" 变体。
...
**Prevention**:
- 生产链采用 Vision 与 Language 独立编译，Runtime 按固定张量接口完成组合；不再保留未完成的统一编译入口
...
**Trigger**: `cd deploy && mkdir build && cd build && cmake .. && make`
...
**Prevention**: Documented in `deploy/CMakeLists.txt` header comment: "S600 symbol homes (verified by nm -D): hbDNN* -> libdnn.so, hbUCP* -> libhbucp.so".
...
**Fix**: Copy the exact flow from HB_HBMRuntime.cc::InferSingleModel — use the macro for defaults, then explicitly set backend. See `deploy/src/hbm_session.cpp::Graph::Execute`.
...
**Fix**: `deploy/run_vision_dummy_test.sh` sets the env var by default (mirroring `run_vlm.sh`). Future LA run scripts (for language and unified) must do the same.
...
**Prevention**: Documented in `deploy/run_vision_dummy_test.sh` header comment + this KNOWN_ISSUES entry. Any future S600 BPU runtime launch script MUST export `HB_DNN_USER_DEFINED_L2M_SIZES` or all 4-core hbms will fail with -200003.
...
**Fix**: 选路线 B — 在 `deploy/src/hbm_session.cpp` 基础上扩展:
...
见 `RUNTIME_ARCHITECTURE.md`。PBD decode 输入 = 1 个上轮尾真实 token +
...
- 文件: `compiler/leap_llm/models/locateanything/text_model_leap.py`
...
**现象**: `build/language.sh` 和
`build/vision.sh` 都传入 mmstar `calib_json_path`，但当前 Language
...
该 skeleton 后续已从生产注册表移除，避免与可部署的独立 Vision/Language 入口混淆。
...
## #030 448 校准张量不能复用于 672 发布 profile (2026-07-18)

**现象**: 早期 MoonViT 图和校准脚本固定为 448x448，接口为
`(1,1024,588) -> (1,256,2048)`；定位任务需要更高空间分辨率，因此发布 profile
调整为 672x672。

**原因**: 672x672 在 patch size 14、2x2 merge 下产生 48x48=2304 个 patch。
视觉 token 数为 576。它与 448 profile 的 patch 数、图像占位 token 数、prefill
激活分布和坐标域均不同，不能用 448 tensor bundle 校准 672 HBM。

**修复**:

- `calibration/prepare.py generate` 默认切换到 672x672；
- 原始图像继续按原分辨率保存，生成阶段默认按比例 letterbox，不再直接拉伸；
- `<box>` 中的二坐标 point 和四坐标 box 同步变换到 letterbox 坐标域；
- 每个 tensor/manifest 保存原图尺寸、缩放、四边 padding、原始 target 和 profile target；
- Vision 编译入口及 LocateAnything Vision 配置默认切换到 672；
- `chunk_size=1024` 保持不变，576 个视觉 token 后最多剩余 448 个文本/template 位置。

**已验证**: 本地 profile 推导得到 Vision 输入 `(1,2304,588)`、输出
